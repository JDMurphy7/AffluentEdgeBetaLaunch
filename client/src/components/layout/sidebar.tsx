...
import { User, StrategyPerformance } from "../../lib/types.js";
...
export default function Sidebar({ userId }: SidebarProps) {
  ...
  return (
    <>
      ...
      <button
        className="fixed top-4 left-4 z-50 md:hidden bg-gold/80 text-black rounded-full p-2 shadow-lg"
      ...
      <aside className={`fixed left-0 top-16 h-full w-64 glass-morphism border-r border-white/20 p-6 sidebar-scrollbar overflow-y-auto hidden md:block`}>
        <div className="space-y-6">
          ...
          <nav className="space-y-2">
            ...
            <Link href="/admin-monitoring" className={`flex items-center space-x-3 p-3 rounded-lg hover:bg-white/10 transition-colors ${location === '/admin-monitoring' ? 'text-gold bg-white/5' : 'text-white'}`}>
      ...
      {/* Mobile Drawer */}
      {open && (
        <div className="fixed inset-0 z-40 bg-black/60 flex md:hidden" onClick={() => setOpen(false)}>
          <aside className="bg-charcoal w-64 h-full p-6 sidebar-scrollbar overflow-y-auto" onClick={e => e.stopPropagation()}>
            <button className="mb-4 text-gold" onClick={() => setOpen(false)} aria-label="Close navigation">
            ...
            {/* Strategy Quick Stats */}
            <div className="glass-morphism p-4 rounded-xl mt-4">
              <h3 className="text-sm font-medium text-gray-300 mb-3">Active Systems</h3>
              <div className="space-y-2">
                {strategies?.slice(0, 3).map((strategy) => (
                  <div key={strategy.strategy.id} className="flex justify-between items-center">
